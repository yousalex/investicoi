-- Update the plan enum to include new plan types
DROP TYPE IF EXISTS public.user_plan CASCADE;
CREATE TYPE public.user_plan AS ENUM ('gratuito', 'mensual', 'trimestral', 'semestral', 'anual');

-- Recreate the profiles table with the new plan enum
ALTER TABLE public.profiles 
  ALTER COLUMN plan TYPE text,
  ALTER COLUMN plan SET DEFAULT 'gratuito';

UPDATE public.profiles SET plan = 'gratuito' WHERE plan IS NULL OR plan = 'free';

ALTER TABLE public.profiles 
  ALTER COLUMN plan TYPE public.user_plan USING plan::public.user_plan;
